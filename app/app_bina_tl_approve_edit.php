   ...
   ?> 
		...
		<section id="main-content" >
			<section class="wrapper" id="section_utama" >
				...
				<form id="form" name="demoform" method="post" action="app_bina_tl_approve_edit.php">
					
					
					<!--input type="hidden" id="sub_menu" name="sub_menu" size="20" value="active_63"-->
					<input type="hidden" name="id_bina" id="id_bina" value="<?php echo $ids ?>">
					<input type="hidden" name="tanggal" id="tanggal" value="<?php echo $tanggal ?>">
					<input type="hidden" name="id_ani" id="ani" value="<?php echo $ani ?>">
					<input type="hidden" name="nd1" id="nd1" value="<?php echo $nd1 ?>">
							...
							<section class="panel">
								<div class="revenue-head" style="background: #695a70;-webkit-border-radius: 4px 4px 0px 0px;border-radius: 4px 4px 0px 0px;  color: #fff;line-height: 50px;">		<?php		
									...
									<span style="float: right;display: inline; padding: 0 10px; font-size: 16px;background: #695a70;">
										<a href="app_bina_tl_approve.php" class="btn btn-warning">Kembali List Approval  </a>  
									</span>
								</div>
								<div class="panel-body">									
									<div class='col-md-4'>
										<div role="form">											
											<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;color:red'>
												<strong>Layanan :</strong> Telkom FBCC
											</p>
											<label class="control-label col-md-12" style="color:blue;"><strong>DETAIL APROVAL :</strong></label>
											...
											<div class="col-md-12">
												<div class="form-group">
													...
													<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;'>
														 ...
														 >> <span style="color:green">Login TL</span> : <span style="color:red"><?php echo $rsk[5];?></span> <br/> 
													...
													<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;'>
														...
														>>  <span style="color:green">Jabatan Penilai</span>   : <span style="color:red"><?php $jb=mysqli_query($conn, "SELECT user3 FROM `cc147_main_users_extended` WHERE `user1`='$rsk[6]'");
														$jbs=mysqli_fetch_row($jb);
														echo $jbs[0]; ?></span>
													</p>													
													<p class="alert alert-warning fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;'>
													...
													<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;'>
														>> <span style="color:green">Tanggal Isi Komitment</span> : <span style="color:red"></span> <br/>
									...
									<div class='col-md-4'>
										<div role="form">
											<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;color:red'>
												<strong>Area :</strong> <?php echo $rsk[15]; ?>
											</p>
											<p>-</p>
								...
								<div class="col-md-4">
										<div role="form">
											<p class="alert alert-info fade in" style='margin-bottom:15px;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;color:purple;color:red'>
												<strong>Segment :</strong> Team Leader
											</p>
											...
											<div class="form-group">													
												...
												<div class="form-group">
													...
													<select class="chosen-select" name="x_id_kat_approve">
														<option value="3">Approve From AGENT</option>
														<option value="0">Rejected From QCO</option>
...
//echo $nilai;
		...
		$x_id_kat_approve = $_POST['x_id_kat_approve'];
		$tanggal = $_POST['tanggal'];
		$id_bina = $_POST['id_bina'];
		$id_ani = $_POST['id_ani'];
		$nd1 = $_POST['nd1'];


		$sql="UPDATE `app_kinerja_nilai` SET `rekomendasi_tl`='$x_rek_tl',`status`='$x_id_kat_approve',`lup_tl`='$lup' WHERE (`tanggal`='$tanggal') AND (`user_id`='$id_bina') AND (`ani` = $id_ani) AND (`nd` = $nd1)";

		echo "$sql";
	
	   	mysqli_query($conn, $sql);

	   	$sql1="UPDATE `app_kinerja_nilai_reject` SET `rekomendasi_tl`='$x_rek_tl',`status`='$x_id_kat_approve',`lup_tl`='$lup' WHERE (`tanggal`='$tanggal') AND (`user_id`='$id_bina') AND (`ani` = $id_ani) AND (`nd` = $nd1)";
