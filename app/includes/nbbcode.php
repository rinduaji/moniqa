...
function bbcode_table($field='message', $form='post', $allowed=0)
{
	...
	<script language="JavaScript" src="includes/javascript/bbcode.js" type="text/javascript"></script>';
		define('BBCODE_JS_ACTIVE', 1);
	}
...
function set_smilies($message, $url='') {
	...
	if (!isset($orig)) {
		...
		for ($i = 0; $i < count($smilies); $i++) {
			...
			$repl[] = '\\1<img src="' . $url . $smilies_path . $smilies[$i]['smile_url'] . '" alt="'.get_code_lang($smilies[$i]['emoticon'],$smilies_desc).'" title="'.get_code_lang($smilies[$i]['emoticon'],$smilies_desc).'" border="0" />\\2';
		}
	}
	if (count($orig)) {
