		...
		switch ($mode)
		{
			case 'add':
				if ($ret = $this->alter_table("$table ADD $field $type")) {
					if (isset($default)) {
						if ($ret = $this->_owner->query("ALTER TABLE $table ALTER COLUMN $field SET DEFAULT '$default'")) {
							$ret = $this->_owner->query("UPDATE $table SET $field = '$default'");
						}
					}
					if ($ret && !$null) {
						$ret = $this->_owner->query("ALTER TABLE $table ALTER COLUMN $field SET NOT NULL");
					}
				}
				return $ret;
			...
			case 'change':
				...
				if ($field[0] == $field[1]) {
					...
					$ret = $this->_owner->query("ALTER TABLE $table RENAME COLUMN $field[0] TO $field[1]");
				}
				if ($ret && $type != '') {
					$ret = $this->_owner->query("ALTER TABLE $table ALTER COLUMN $field[1] TYPE $type");
				}
				if ($ret && isset($default)) {
					$ret = $this->_owner->query("UPDATE $table SET $field[1] = '$default'");
				}
				return $ret;
		}
	}
