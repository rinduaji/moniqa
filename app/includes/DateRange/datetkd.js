...
var gBegin=gTomorrow;	// calendar date range begin from [Year,Month,Date]. Using gToday here will make it start from today.
var gEnd=[2030,12,31];	// calendar date range end at [Year,Month,Date]
var gsOutOfRange="Sori yo gak iso...";	// out-of-date-range error message. If set to "", no alerts will popup on such error.
var guOutOfRange="Cross.gif";	// the background image url for the out-range dates. e.g. "outrange.gif"
...
var gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"Anda tidak dapat memilih hari ini !\");return false;' onmouseover='return true;' ><b>Hari ini : "+gToday[2]+" "+gMonths[gToday[1]-1]+" "+gToday[0]+"</b></A>";
...
var gcBGToday="#aff0C8";	// the background color for today 
var guTodayBGImg="";	// url of image as today's background
var giMarkToday=1+2+4+8; // Effects for today - 0: nothing; 1: set background color with gcBGToday; 2: draw a box with gcBGToday; 4: bold the font; 8: set font color with gcFGToday; 16: set background image with guTodayBGImg; - they can be added up to create mixed effects.
var gsTodayTip="Tanggal sekarang";	// tooltip for today
...
var gsSelectedTip="Tanggal Pilihan";	// tooltip for selected dates
...
var gAgendaMask=[-0,-1,gcCellBG,"black",-1,false,null];	// [message, action, bgcolor, fgcolor, bgimg, boxit, html] - Set the relevant bit to -1 to keep the original agenda/event value of that bit intact. Any other value will be used to override the original one defined in agenda.js. Check the tutorial for details.
