 ...
 *
 *	 $Id: functions_topics_list.php,v 9.4 2005/10/11 12:31:52 djmaze Exp $
 *
 *
 ***************************************************************************/
	...
	for ($i=0; $i < count($topic_rowset); $i++)
	{
				...
				if( $topic_rowset[$i]['post_time'] > $userdata['user_lastvisit'] )	{
					if( !empty($tracking_topics) || !empty($tracking_forums) || !empty($tracking_all) )
					{
						...
						if ( $unread_topics )
						{
							...
							$newest_post_img = '<a href="' . getlink("&amp;file=viewtopic&amp;" . POST_TOPIC_URL . "=$topic_id&amp;view=newest") . '"><img src="' . $images['icon_newest_reply'] . '" alt="' . $lang['View_newest_post'] . '" title="' . $lang['View_newest_post'] . '" border="0" /></a> ';
						}
						else
						{
							$folder_image = $folder;
		...
		switch ($topic_item_type)
		{
			...
			default:
				...
				$last_post_url		= '<a href="' . getlink("&amp;file=viewtopic&amp;"	. POST_POST_URL . '=' . $topic_rowset[$i]['topic_last_post_id']) . '#' . $topic_rowset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
