 														...
 														<tbody>
 															<?php
																...
																// while ($qq <= date('m')) {
                                                                    ...
                                                                    $query_xena = "
																	(SELECT t.UPD_AGENT,t.NAMA_MASTER,t.ALAMAT_MASTER,t.NO_KONTAK,t.TGL_CALL,t.HUB_PENERIMA,t.PENERIMA,t.TRANS_ID,td.KENDALA,t.GIMMICK,tnd.FCR,td.TODO,t.KETERANGAN,td.ND,td.TGL_JANJI_BAYAR FROM trans_history_202307 AS t INNER JOIN trans_detail_history_202307 AS td ON t.TRANS_ID = td.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON t.TRANS_ID = tnd.TRANS_ID 
																	...
																	FROM trans_history_202307 AS d  
																	JOIN (
																	SELECT max(d.NO_KONTAK) AS m, d.UPD_AGENT
																	FROM trans_history_202307 as d
																	JOIN (
																	SELECT max(NO_KONTAK) AS m, UPD_AGENT
																	FROM trans_history_202307 WHERE STATUS_CALL='CONTACTED' AND REASON_CALL ='SETUJU' AND LUP between '$range_tanggal1' 
																	and '$range_tanggal2' 
																	...
																	ORDER BY UPD_AGENT,NO_KONTAK ASC) AS f JOIN trans_detail_history_202307 AS ttd ON f.TRANS_ID= ttd.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON ttd.TRANS_ID = tnd.TRANS_ID  GROUP BY f.TRANS_ID ORDER BY f.UPD_AGENT ASC)";
