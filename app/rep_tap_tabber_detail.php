...
<section id="main-content">
	...
	<section class="wrapper">
		...
		<form name="form1" method="post" action="rep_tap_tabber_detail.php?login_tabber=<?php echo $_GET['login_tabber'];?>&area=<?php echo $_GET['area'];?>&bulan=<?php echo $_GET['bulan'];?>&tahun=<?php echo $_GET['tahun'];?>&agent=<?php echo $user;?>" >
			...
			<div class="row">
				...
				<div class="col-lg-12">
					...
					<section class="panel">
                      ...
                      //echo date('m');
																$area_id = $_GET['area'];
																$login_tabber = $_GET['login_tabber'];
																$qq=date('m');
																$q=date('Y');
																$month = $_GET['bulan'];
																$year = $_GET['tahun'];
...
// $sql=mysqli_query($conn,"Select area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area");
																		$sql=mysqli_query($conn,"Select kn.tanggal,kn.ani,u.user7,kn.lup_tl_name,kn.lup_qa,kn.proses_layanan,kn.sikap_layanan,kn.user_id FROM app_kinerja_nilai  as kn INNER JOIN cc147_main_users_extended as u ON kn.lup_qa=u.user1  WHERE MONTH(kn.tanggal) = '$qq' AND YEAR(kn.tanggal) = '$q' AND u.user7='$area_id' AND kn.lup_qa='$login_tabber'");
...
// $sql=mysqli_query($conn,"Select area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area");
									$sql=mysqli_query($conn,"Select kn.tanggal,kn.ani,u.user7,kn.lup_tl_name,kn.lup_qa,kn.proses_layanan,kn.sikap_layanan,kn.user_id FROM app_kinerja_nilai  as kn INNER JOIN cc147_main_users_extended as u ON kn.lup_qa=u.user1  WHERE MONTH(kn.tanggal) = '$qq' AND YEAR(kn.tanggal) = '$q' AND u.user7='$area_id' AND kn.lup_qa='$login_tabber' AND kn.user_id='$user'");
 //echo "Select  area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area";
									while($row=mysqli_fetch_row($sql)){
										$total_nilai = $row[5] + $row[6];
										?>
